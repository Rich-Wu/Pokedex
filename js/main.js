...
class Trainer {
    ...
    get(name) {
        for (let poke in this.pokes) {
            if (
                this.pokes[poke]["species"] == name ||
                this.pokes[poke]["number"] == name
            ) {
    ...
    getAbilities() {
        ...
        for (let i = 1; i < this.abilities.length; i++) {
            abilityString =
                abilityString +
                ", " +
                capitalize(this["abilities"][i]["ability"]["name"]);
        }
...
function addPokemon() {
    ...
    xhttp.open(
        "GET",
        "https://pokeapi.co/api/v2/pokemon/" + userEntry + "/",
        true,
    );
    xhttp.send();
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4 && this.status != 200) {
            alert(
                "An error occurred while processing your input of " +
                    userEntry +
...
function addFlavor(pokemon) {
    ...
    xhttp2.open(
        "GET",
        "https://pokeapi.co/api/v2/pokemon-species/" + pokemon.number + "/",
        true,
    );
    xhttp2.send();
...
function drawPokeballs() {
    ...
    for (pokemon in Red.pokes) {
        if (Red.pokes[pokemon]["caught"] == false) {
            ...
            document
                .getElementById("pokeSelector")
                .insertBefore(
                    newMonster,
                    document.getElementsByClassName("carousel-item")[0],
                );
...
function removePokemon() {
    ...
    Red["pokes"][
        document.getElementsByClassName("active")[0].firstElementChild.id
    ].dexEntry();
}
...
document
    ...
    .addEventListener("click", removePokemon);
fetchPokemon("vulpix");
